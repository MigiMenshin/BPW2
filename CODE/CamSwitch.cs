...
public class CamSwitch : MonoBehaviour
{
    ...
    private void Awake()
    {
        // Add the camera to the list of all cameras
        if (!allCams.Contains(activeCam))
        {
    ...
    private void SetActiveCamera()
    {
        ...
        followCam.Priority = 0;  // Set follow camera priority to 0
        Debug.Log("Active camera set to: " + activeCam.name);
    }

    private void ResetFollowCamera()
    {
        // Reset the follow camera priority
        followCam.Priority = 10;
        activeCam.Priority = 0;
